...
use WHMCS\Database\Capsule; # TODO: Migrate this to the DB class
    ...
    for ($i = 0, $iMax = count($domains); $i < $iMax; $i++) {
        ...
        if ($tld == ".fr" || $tld == ".re" || $tld == ".pm" || $tld == ".yt" || $tld == ".wf" || $tld == ".tf") {
            ...
            if ($additionalFields["legalentity"] == "7. foreigners who match 2 - 6") {
                $euroCountries = ["AT", "BE", "BG", "HR", "CY", "CZ", "DK", "EE", "FI", "FR", "DE", "GR", "HU", "IS", "IE", "IT", "LV", "LI", "LT", "LU", "MT", "NL", "NO", "PL", "PT", "RO", "SM", "SK", "SI", "ES", "SE", "CH", "GB", "VA"];
...
function hook_ibs_validateData($params)
{
    ...
    $euroCountries = ["AX", "AT", "BE", "BG", "HR", "CY", "CZ", "DK", "EE", "FI", "FR", "GF", "DE", "GI", "GR", "GP", "HU", "IS", "IE", "IT", "LV", "LI", "LT", "LU", "MT", "MQ", "NL", "NO", "PL", "PT", "RE", "RO", "SK", "SI", "ES", "SE", "GB"];
...
function hook_ibs_check_trademark_claim($vars)
{
    ...
    for ($domainCnt = (count($domains) - 1); $domainCnt >= 0; $domainCnt--) {
        ...
        if ($domain["type"] == "register") {
            $params = ibs_getapiDetails(); // load registrar settings instead
...
//      $response = mysql_fetch_array($result);
        ...
        for ($cartcnt = 0; $cartcnt < count($sessionDomains); $cartcnt++) {
            if ($sessionDomains[$cartcnt]["domain"] == $response->domain && isset($sessionDomains[$cartcnt]["TMCH"])) {
                ...
                $values = [
                    "domainid" => $domains[$domainCnt],
                    "name" => "tmchid",
                    "value" => $sessionDomains[$cartcnt]["tmchid"]
                ];
                ...
                $values = [
                    "domainid" => $domains[$domainCnt],
                    "name" => "tmchnotafter",
                    "value" => $sessionDomains[$cartcnt]["tmchnotafterdate"]
                ];
                Capsule::table($table)->insert($values);
                //$newId = insert_query($table, $values);
                ...
                $values = [
                    "domainid" => $domains[$domainCnt],
                    "name" => "tmchaccepteddate",
                    "value" => $sessionDomains[$cartcnt]["accepteddate"]
                ];
                Capsule::table($table)->insert($values);
                //$newId = insert_query($table, $values);
            }
...
{
    $tldList = [".com", ".eu", ".fr", ".re", ".pm", ".yt", ".wf", ".tf", ".uk", ".co.uk", ".org.uk", ".me.uk", ".nl", ".asia", ".de", ".it", ".nyc", ".tel"];
 ...
 */
function ibs_domainMenuUpdate($vars)
{
    // \WHMCS\Domain\Domain
    $domain = Menu::context("domain");

    $menu = $vars["primarySidebar"]->getChild("Domain Details Management");
    if (is_null($menu) || $domain->registrar !== "ibs") {
        return $vars;
    }

    $registrar = new \WHMCS\Module\Registrar();
    if (!$registrar->load("ibs")) {
        return $vars;
    }

    // fetch TLD Info
    $result = ibs_call($registrar->getSettings(), "Domain/Check", [
...
add_hook("ClientAreaPageDomainDNSManagement", 1, "ibs_domainMenuUpdate");
add_hook("ClientAreaPageDomainEPPCode", 1, "ibs_domainMenuUpdate");
add_hook("ClientAreaPageDomainContacts", 1, "ibs_domainMenuUpdate");
add_hook("ClientAreaPageDomainRegisterNameservers", 1, "ibs_domainMenuUpdate");
add_hook("ClientAreaPageDomainDetails", 1, "ibs_domainMenuUpdate");
